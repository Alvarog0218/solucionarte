    ...
    <section className="py-16 md:py-24 bg-[#051316] overflow-hidden" id="proceso">
      <div className="max-w-[1280px] mx-auto px-5 md:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="inline-block bg-[#0e3b44] text-[#bbf2fc] px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-3 border border-[#143941]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative mb-12">
          ...
          <div className="hidden md:block absolute top-[36px] left-[12%] w-[76%] h-[2px] bg-[#143941]/50 -z-0"></div>
            ...
            return (
              <div
                ...
                className={`relative z-10 flex flex-col items-center text-center p-6 rounded-2xl transition-all cursor-pointer border ${
                  isActive
                    ? 'bg-[#0c242b] border-[#0097b2] shadow-lg scale-102'
                    : 'bg-[#0c242b]/60 border-[#143941] hover:bg-[#0c242b] hover:border-[#8fa3a7]/30'
                }`}
              >
                <div
                  className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg mb-4 transition-colors shadow-md ${
                    isActive ? 'bg-[#0097b2] text-white' : 'bg-[#0e3b44] text-[#bbf2fc]'
                  }`}
                >
                  {step.number}
                </div>

                <h3 className="font-['Outfit'] font-bold text-lg text-[#e6eff1] mb-1">
                  {step.title}
                </h3>
                <p className="text-xs text-[#8fa3a7] mb-3">{step.subtitle}</p>
        ...
        <div className="bg-[#0c242b] rounded-3xl p-8 md:p-10 border border-[#143941] shadow-sm max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <span className="w-8 h-8 rounded-full bg-[#0e3b44] text-[#bbf2fc] font-bold text-sm flex items-center justify-center">
                {steps[activeStep].number}
              </span>
              <h4 className="font-['Outfit'] text-2xl font-bold text-[#e6eff1]">
                Paso {steps[activeStep].number}: {steps[activeStep].title}
              </h4>
            </div>

            <p className="text-sm text-[#8fa3a7] mb-6 leading-relaxed font-medium">
              {steps[activeStep].summary}
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-3.5 bg-[#051316] rounded-xl border border-[#143941]">
                <span className="font-bold text-[#0097b2] block mb-1">Entregable Concreto:</span>
                <span className="text-[#8fa3a7]">{steps[activeStep].deliverable}</span>
              </div>
              <div className="p-3.5 bg-[#051316] rounded-xl border border-[#143941]">
                <span className="font-bold text-[#0097b2] block mb-1">Tiempo Estimado:</span>
                <span className="text-[#8fa3a7]">{steps[activeStep].duration}</span>
              </div>
            </div>
          </div>
