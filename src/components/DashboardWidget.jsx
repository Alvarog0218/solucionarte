...
export default function DashboardWidget() {
  ...
  return (
    <div className="w-full max-w-xl mx-auto relative">
      ...
      <div className="absolute -top-6 -left-6 w-32 h-32 bg-[#b1f0ce] rounded-full blur-3xl opacity-40 pointer-events-none"></div>
      <div className="absolute -bottom-10 -right-10 w-44 h-44 bg-[#d9e6da] rounded-full blur-3xl opacity-50 pointer-events-none"></div>

      {/* Main Container Card */}
      <div className="bg-white rounded-2xl shadow-xl border border-[#E2ECE9] overflow-hidden relative z-10 hover-lift">
        ...
        <div className="bg-[#e9e8e6]/80 px-4 py-3 border-b border-[#bfc9c1]/40 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 text-[11px] font-medium text-[#2d6a4f] bg-[#d9e6da] px-2 py-0.5 rounded-md">
              <span className="w-1.5 h-1.5 rounded-full bg-[#2d6a4f] animate-ping"></span>
              En Ejecución
            </span>
          </div>
        </div>
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="font-['Outfit'] text-[#0f5238] text-xl font-bold flex items-center gap-2">
            ...
            <button className="p-2 rounded-lg bg-[#f4f3f1] text-[#556158] hover:text-[#0f5238] transition-colors relative">
              <Bell className="w-4 h-4" />
              <span className="absolute top-1 right-1 w-2 h-2 rounded-full bg-[#2d6a4f]"></span>
            </button>
          </div>

          {/* Interactive Automation Status Bar */}
          <div className="p-4 bg-[#faf9f7] rounded-xl border border-[#E2ECE9] mb-5 flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3.5">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
                automationActive ? 'bg-[#2d6a4f] text-white shadow-sm' : 'bg-[#e9e8e6] text-[#556158]'
              }`}>
              ...
              <div>
                ...
                <p className="text-xs text-[#556158] flex items-center gap-1">
                  <span>Estado:</span>
                  <span className={`font-semibold ${automationActive ? 'text-[#0f5238]' : 'text-amber-700'}`}>
            ...
            <button
              ...
              className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors focus:outline-hidden cursor-pointer ${
                automationActive ? 'bg-[#2d6a4f]' : 'bg-[#bfc9c1]'
              }`}
          ...
          {/* Tab Navigation */}
          <div className="flex border-b border-[#E2ECE9] mb-5 gap-4 text-xs font-semibold">
            <button
              onClick={() => setActiveTab('summary')}
              className={`pb-2 transition-colors cursor-pointer border-b-2 ${
                activeTab === 'summary' ? 'border-[#0f5238] text-[#0f5238]' : 'border-transparent text-[#556158] hover:text-[#1a1c1b]'
              }`}
            >
            ...
            <button
              ...
              className={`pb-2 transition-colors cursor-pointer border-b-2 flex items-center gap-1.5 ${
                activeTab === 'live-feed' ? 'border-[#0f5238] text-[#0f5238]' : 'border-transparent text-[#556158] hover:text-[#1a1c1b]'
              }`}
            >
          ...
          {activeTab === 'summary' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#f4f3f1] rounded-xl p-3.5 border border-[#E2ECE9]/60 hover:bg-[#d9e6da]/30 transition-colors">
                  ...
                  <p className="text-[11px] text-[#5b675e] mt-1">Sin retrasos en respuesta</p>
                </div>
                ...
                <div className="bg-[#f4f3f1] rounded-xl p-3.5 border border-[#E2ECE9]/60 hover:bg-[#d9e6da]/30 transition-colors">
                  ...
                  <p className="text-[11px] text-[#5b675e] mt-1">Por cada colaborador</p>
                </div>
              </div>

              {/* Realtime Action Card */}
              <div className="p-3.5 bg-[#d9e6da]/40 rounded-xl border border-[#bfc9c1]/50 flex items-start gap-3">
          ...
          {activeTab === 'live-feed' && (
            <div className="space-y-2.5">
              <div className="p-3 bg-[#faf9f7] rounded-lg border border-[#E2ECE9] flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping"></span>
                  <span className="font-bold text-[#1a1c1b]">Total Operaciones Atendidas:</span>
                </div>
                <span className="font-mono font-bold text-[#0f5238] bg-[#d9e6da] px-2 py-0.5 rounded-md text-xs">
                  {processedCount} solicitudes
                </span>
              </div>

              <div className="text-xs text-[#556158] space-y-2">
                <div className="p-2.5 bg-white border border-[#E2ECE9] rounded-lg flex items-center gap-2 shadow-2xs">
          ...
          <div className="mt-5 pt-4 border-t border-[#E2ECE9] flex items-center justify-between text-[11px] text-[#556158]">
            <span className="flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-[#2d6a4f]" />
              Seguridad de Grado Empresarial
            </span>
            <span className="font-medium text-[#2d6a4f] hover:underline cursor-pointer flex items-center gap-0.5">
